...
<?php                /** <form enctype="multipart/form-data" action="<?php URL ;?> index/createaccount" method="post" class="p-6 space-y-4 relative bg-white shadow-lg rounded-lg">**/ ?>
  ...
  border-spacing: 30 50px;">
                        <thead ><tr><td>Transaction Date</td> <td>Plot</td> <td>Paid</td></tr></thead>
                      <tbody>
                        <?php 
                        $tamount=0;
                          foreach ($this->ff as $key => $value) {
                              $tamount=$tamount+$value['credit'];
                              $name=$value['client_name'];
                              $mobile=$value['client_mobile'];
                           echo '<tr>
                                  <td style="text-align:left;">'. $value["created_at"] .'</td>
                                  <td style="text-align:right;">'. $value["qty"] .'</td>
                                  <td style="text-align:right;">=N='. number_format($value["credit"]) .'</td>
                        ...
                        ?>
                        <tfoot><tr><td><?= $name ;?></td><td><?= $mobile ;?></td><td style="text-align: right;"><?= number_format($tamount) ;?></td></tr></tfoot>
                      </tbody>
