            ...
            <div class="sidebar_inner" data-simplebar>
                ...
                <ul>
                   ...
                   <?php
                    ...
                    ?>    
                    <li><a href="<?php echo URL."sendindsms" ?>">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="text-pink-500">
                            <path fill-rule="evenodd" d="M6 6V5a3 3 0 013-3h2a3 3 0 013 3v1h2a2 2 0 012 2v3.57A22.952 22.952 0 0110 13a22.95 22.95 0 01-8-1.43V8a2 2 0 012-2h2zm2-1a1 1 0 011-1h2a1 1 0 011 1v1H8V5zm1 5a1 1 0 011-1h.01a1 1 0 110 2H10a1 1 0 01-1-1z" clip-rule="evenodd" />
                            <path d="M2 13.692V16a2 2 0 002 2h12a2 2 0 002-2v-2.308A24.974 24.974 0 0110 15c-2.796 0-5.487-.46-8-1.308z" />
            ...
            <div class="mcontainer">
                ...
                <div class="lg:flex lg:space-x-12">
                    ...
                    <div class="lg:w-1/4 flex-shrink-0 space-y-5">                            
                      ...
                      <h2 class="text-xl font-semibold mt-7"> Send Curent Status(SMS) </h2>
                      <div class="card">
                          ...
                          <?php
                         // require_once  ('models/sendsmsdetails_model.php');
                          //$d= new Dashboard_Model();
                         

                         // echo "<pre>";
                         // print_r($this->getnoOfworkingDays);
                          //echo "<pre>";
                          //print_r($this->dateofcontractactivation);


                          foreach ($this->allmyproducts as $key => $value) {
                           // echo $key."<br/>";
                            $activeurl="sendsmsdetails/sendmsg".$key;
                            $orderno=0;
                            $numofdays=0;
                            $ppday=0;
                            $expPaidCost=0;
                            $description="Dear Client your land purchased at ".$value["pname"];
                                    foreach ($this->getnoOfworkingDays as $key1 => $value1) {
                                        $orderno=($value1['orderno']);
                                        $numofdays=$value1['number_ofdays'];

                                        //echo $orderno." ok  ". $numofdays;
                                        //exit();
                                            if($orderno==$value['orderno']){

                                                
                                                foreach ($this->dateofcontractactivation as $key2 => $value2) {
                                                    # code...
                                                    if($orderno==(substr($value2['orderno'], -8))){
                                                        $ppday=$value2['daily'];
                                                    }
                                                }
                                                $expPaidCost=$ppday * $numofdays;
                                                $due=$value['amountpaid']-$expPaidCost;
                                                $plots=$value['qty']."plot(s)";
                                                $Tamount=$value['totalamt'];
                                                $paid=$value['amountpaid'];
                                                $balance=$Tamount-$paid;
                                                $message = "Dear Client,your land:" . $plots . ",Cost=N" . number_format($Tamount) . ",Paid=N" . number_format($paid) . ",Balance=N" . number_format($balance) . ",Kindly pay an accumulated amount of N" . number_format($due) . " due out of your balance.Thanks.DreamCityHES Ltd";

                                                $messageNew = "Dear Client,your land:" . $plots . ",Cost=N" . number_format($Tamount) . ",Paid=N" . number_format($paid) . ",Balance=N" . number_format($balance) . ", Please make additional payment.Thanks.DreamCityHES Ltd";

                                                $messageMonth = "Dear Client,as at Dec 1,2023 your land:" . $plots . ",Cost=N" . number_format($Tamount) . ",Paid=N" . number_format($paid) . ",Balance=N" . number_format($balance) . ", Happy New Month Thanks.DreamCityHES Ltd";


                                                echo'
                                                    <form enctype="multipart/form-data" action='.$activeurl.' method="post" >
                                                       <textarea  name="message" rows="4" cols="50">
                                                        '.$message.'
                                                        </textarea>

                                                        <textarea  name="messageNew" rows="4" cols="50">
                                                        '.$messageNew.'
                                                        </textarea>

                                                         <textarea  name="messageMonth" rows="4" cols="50">
                                                        '.$messageMonth.'
                                                        </textarea>

                                                        

                                                        <input type="submit" value="SEND SMS NOW!">
